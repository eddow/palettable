...
// Dragging a tool while hovering a track gap must light the two
// flanking stack gaps: pointerdown on an edit-mode guard, hover a track
// gap (not a toolbar), and the border's stack spaces paint
// `highlighted` (core `stackFlanks` derivation on every in-track hover).
//
...
// gap stays clear of the overlay the whole way.
test('dragging a tool over a track highlights the flanking stack gaps', async ({ page }) => {
	...
	try {
		...
		// Hover the track gap: inside the track but outside any toolbar
		// (the leading track gap owns the free height).
		const steps = 12
		for (let i = 1; i <= steps; i += 1) {
			const x = gapBox!.x + gapBox!.width / 2
			const y = gapBox!.y + gapBox!.height / 2
			const sx =
		...
		await expect
			...
			// Both flanking gaps paint — a single gap means the track-gap
