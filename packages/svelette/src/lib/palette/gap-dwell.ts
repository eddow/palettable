/**
 * Shared hover-dwell state machine for stack gaps (border stacks + parking).
 *
 * Both `ToolbarBorder` and `Parking` run the same protocol:
 *
 ...
 *
 * Plain class (no runes): the two components own their `$state` fields and
 * delegate transitions here, keeping the timer/latch rules in one place.
 * `isHighlighted` stays in the component (border vs parking veto + mask
 * rules differ); the dwell only asks it whether the arming gap may fire.
 */
...
export class GapDwell {
	...
	/**
	 * Evaluate the arming effect. Call from a `$effect` keyed on
	 * `hovered`/drag/mask state; returns a cleanup that cancels the timer.
	 *
	 * - `canArm(gap)` — the component's highlight predicate for the gap
	 * - `onFire(gap)` — the commit (must re-check hover liveness itself;
	 *   async slide-arming after `tick()` lives there too)
	 */
	arm(
		state: GapDwellState,
		options: {
