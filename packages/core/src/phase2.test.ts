...
import { createGapDwellState, GapDwell, type GapDwellState } from './gap-dwell.js'
import {
	actualTrackSpaceAt,
	type Border,
	type Borders,
...
describe('GapDwell', () => {
	function armed(state: GapDwellState, gap: number): GapDwellState {
		state.hovered = gap
		return state
	}

	it('fires once after the dwell while held', () => {
		vi.useFakeTimers()
		try {
			const dwell = new GapDwell()
			const state = armed(createGapDwellState(), 1)
			const onFire = vi.fn()
			const canArm = () => true
			dwell.arm(state, { editing: true, dragging: true, masked: false, canArm, onFire })
			expect(onFire).not.toHaveBeenCalled()
			vi.advanceTimersByTime(configuration.stackDzHoverMs)
			expect(onFire).toHaveBeenCalledTimes(1)
			expect(onFire).toHaveBeenCalledWith(1)
			// Holding on the same gap does not re-arm (one-shot latch).
			dwell.arm(state, { editing: true, dragging: true, masked: false, canArm, onFire })
			vi.advanceTimersByTime(configuration.stackDzHoverMs * 3)
			expect(onFire).toHaveBeenCalledTimes(1)
		} finally {
			vi.useRealTimers()
		}
	...
	it('cancels when the pointer leaves before the dwell', () => {
		vi.useFakeTimers()
		try {
			const dwell = new GapDwell()
			const state = armed(createGapDwellState(), 0)
			const onFire = vi.fn()
			dwell.arm(state, {
				editing: true,
				dragging: true,
			...
			dwell.hoverGap(state, undefined)
			dwell.arm(state, {
				editing: true,
				dragging: true,
				masked: false,
	...
	it('retargets when the pointer moves to another gap', () => {
		vi.useFakeTimers()
		try {
			const dwell = new GapDwell()
			const state = armed(createGapDwellState(), 0)
			...
			dwell.arm(state, options)
			vi.advanceTimersByTime(configuration.stackDzHoverMs / 2)
			dwell.hoverGap(state, 1)
			dwell.arm(state, options)
			vi.advanceTimersByTime(configuration.stackDzHoverMs / 2)
			expect(onFire).not.toHaveBeenCalled()
			vi.advanceTimersByTime(configuration.stackDzHoverMs / 2)
	...
	it('never arms when editing/dragging is off or the gap is masked', () => {
		vi.useFakeTimers()
		try {
			const dwell = new GapDwell()
			const onFire = vi.fn()
			for (const options of [
				{ editing: false, dragging: true, masked: false },
				{ editing: true, dragging: false, masked: false },
				{ editing: true, dragging: true, masked: true },
			]) {
				const state = armed(createGapDwellState(), 0)
				dwell.arm(state, { ...options, canArm: () => true, onFire })
				vi.advanceTimersByTime(configuration.stackDzHoverMs * 2)
			}
			expect(onFire).not.toHaveBeenCalled()
		} finally {
	...
	it('reset clears hover, latch, and timer', () => {
		vi.useFakeTimers()
		try {
			const dwell = new GapDwell()
			const state = armed(createGapDwellState(), 0)
			...
			dwell.reset(state)
			expect(state).toEqual({ active: undefined, hovered: undefined, committed: undefined })
			vi.advanceTimersByTime(configuration.stackDzHoverMs * 2)
			expect(onFire).not.toHaveBeenCalled()
		} finally {
			vi.useRealTimers()
